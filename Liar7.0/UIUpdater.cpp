...
// 新增：展示点餐结果，并切换场景
void MainWindow::showOrderResult(int itemIndex) {
    if (!orderResultImageLabel) return;

    // 修复：通过数学计算生成目标序号 1->11, 2->21... 避免 Qt 的 %11 解析歧义
    int targetIndex = itemIndex * 10 + 1;
    QString resultImgName = QString("/image-%1.png").arg(targetIndex);
    QString resultImgPath = QCoreApplication::applicationDirPath() + resultImgName;

    QPixmap pix(resultImgPath);
    if (!pix.isNull()) {
        orderResultImageLabel->setPixmap(pix);
        orderResultImageLabel->setText("");
    } else {
        orderResultImageLabel->setText(QString("【图片缺失: image-%1.png】").arg(targetIndex));
        orderResultImageLabel->setStyleSheet("color:#ff5555; font-size:30px; font-weight:bold; background:black;");
    }

    stack->setCurrentWidget(orderResultScene);
}
