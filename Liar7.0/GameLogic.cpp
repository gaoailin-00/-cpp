...
// 根据NPC名称和出牌数量返回质疑概率 (0-100)
int MainWindow::getNpcChallengeRate(const QString& npcName, int cardCount) {
    if (npcName == "牧场主") {
        if (cardCount == 1) return 20;
        if (cardCount == 2) return 35;
        if (cardCount == 3) return 60;
        if (cardCount == 4) return 85;
        if (cardCount >= 5) return 100;
    } else if (npcName == "老警长") {
        if (cardCount == 1) return 25;
        if (cardCount == 2) return 40;
        if (cardCount == 3) return 70;
        if (cardCount == 4) return 90;
        if (cardCount >= 5) return 100;
    } else if (npcName == "华工老板") {
        if (cardCount == 1) return 35;
        if (cardCount == 2) return 50;
        if (cardCount == 3) return 75;
        if (cardCount >= 4) return 100;
    }
    return 50; // 默认50%
}

void MainWindow::startBattle(const QString& npcName) {//对局开始
...
void MainWindow::handleRoundWin(const QString& message) {
    ...
    QTimer::singleShot(2000, this, [this]() {//游戏胜利判定
...
void MainWindow::handleRoundLose(const QString& message) {//单局失败判定
    playerWonLastGame = false;
    turnState = BattleEnded;
    updateActionButtons();
...
void MainWindow::doPlayerPlayCard() {//对局中操做
    ...
    // 根据出牌数和NPC名称计算质疑概率
    int challengeRate = getNpcChallengeRate(currentNpc.name, selected.size());
    bool npcChallenge = (QRandomGenerator::global()->bounded(100) < challengeRate);
...
void MainWindow::resolveNpcChallenge() {//被质疑
...
void MainWindow::doNpcPlayCard() {
    ...
    int maxPlay = qMin(5, currentNpc.cardCount());
...
void MainWindow::resolvePlayerChallenge() {//质疑对方
...
void MainWindow::doPlayerPass() {//玩家不质疑
    if (turnState != WaitingPlayerDecision) return;
